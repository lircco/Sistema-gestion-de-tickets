...
function Index() {
  ...
  const [, setRole] = useState<Role>("alumno");
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      {view === "login" && (
        <LoginScreen
          accounts={accounts}
          onRegister={(acc) => setAccounts((prev) => [...prev, acc])}
          onLogin={(acc) => {
            setRole(acc.role);
            setCurrentUser(acc);
      ...
      {view === "user" && (
        <UserDashboard
          ...
          user={currentUser!}
          tickets={userTickets}
          onCreateTicket={(t) => setUserTickets((prev) => [t, ...prev])}
          mode={mode}
          onToggleMode={toggleMode}
        />
      )}
...
function LoginScreen({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (tab === 0) {
      ...
      const acc: Account = { email, password, role: "alumno", name };
      onRegister(acc);
  ...
  return (
    <Box
      sx={{
        ...
        p: 2,
        background:
          "linear-gradient(135deg,#fafbfc 0%,#fafbfc 55%,#eef2f7 55%,#eef2f7 100%)",
      }}
    >
        ...
        <Card sx={{ width: "100%", p: 3, boxShadow: "0 10px 40px rgba(0,0,0,0.06)" }}>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            ...
            <Button
              ...
              variant="contained"
              size="large"
              sx={{ py: 1.4, fontWeight: 700, letterSpacing: 1 }}
            >
              {tab === 1
                ? "CREAR CUENTA"
                : role === "admin"
  ...
  const sidebar = (
    ...
    >
      ...
      <Stack spacing={0.5}>
        ...
        <SideAction
          icon={<LogoutOutlined sx={{ color: "#dc2626" }} />}
          label="Cerrar Sesión"
          danger
          onClick={onLogout}
        />
...
function AdminDashboard({
  ...
  const [openTicket, setOpenTicket] = useState<null | { id: string; title: string }>(null);
  const items = [
  ...
  return (
    <AppShell
      ...
      onLogout={onLogout}
      user={{ name: admin.name, role: "Administrador · Soporte UNRaf" }}
      mode={mode}
    ...
    >
      ...
      {active === "tickets" && openTicket && (
        <TicketDetail ticket={openTicket} onBack={() => setOpenTicket(null)} admin={admin} />
      )}
      {active === "reports" && <ReportsSection />}
      {active === "areas" && <AreaManagementSection />}
      {active === "kb" && <KnowledgeBaseSection />}
      {active === "settings" && (
        <SettingsSection person={admin} mode={mode} onToggleMode={onToggleMode} legajo="2025-000142" />
      )}
...
function AdminHome({ onOpenTicket }: { onOpenTicket: (t: { id: string; title: string }) => void }) {
  return (
    <Stack spacing={3}>
      <Box>
...
function StatCard({
  color, icon, value, label, chip, chipColor, chipText,
}: {
  color: string; icon: React.ReactNode; value: string; label: string;
  chip: string; chipColor: string; chipText: string;
}) {
  return (
...
function TicketsTable({ onOpenTicket }: { onOpenTicket: (t: { id: string; title: string }) => void }) {
  const rows = [
      ...
      <Paper sx={{ p: 2, overflowX: "auto" }}>
        ...
        >
          <TextField
            ...
            size="small"
            fullWidth
            slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchOutlined sx={{ color: "#9aa4b2" }} /></InputAdornment> } }}
          />
...
function UserDashboard({
  ...
  user: Account;
  tickets: UserTicket[];
  onCreateTicket: (t: UserTicket) => void;
  mode: "light" | "dark";
  onToggleMode: () => void;
}) {
  const [active, setActive] = useState("dashboard");
  ...
  return (
    <>
      <AppShell
        ...
        onSelect={setActive}
        onLogout={onLogout}
        user={{ name: user.name, role: "Alumno UNRaf" }}
        mode={mode}
        onToggleMode={onToggleMode}
      >
        {active === "dashboard" && (
        ...
        {active === "settings" && (
          <SettingsSection person={user} mode={mode} onToggleMode={onToggleMode} legajo="2024-001284" />
        )}
      </AppShell>
      <NewTicketDialog
        open={openNew}
      ...
      <Stack direction={{ xs: "column", md: "row" }} spacing={2.5}>
        <Paper
          ...
          sx={{
            ...
            minHeight: 200,
            backgroundImage:
              "linear-gradient(135deg, rgba(10,61,98,0.92), rgba(10,61,98,0.85))",
            cursor: "pointer",
          }}
        >
...
function UserTicketsTable({ tickets, onOpenNew }: { tickets: UserTicket[]; onOpenNew: () => void }) {
  return (
    <Stack spacing={3}>
      ...
      <DialogContent dividers>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <TextField
            ...
            size="small"
          >
            {["Tecnología", "Académica", "Sistemas", "Infraestructura", "Trámites"].map((c) => (
                ...
                {files.map((f, i) => (
                  ...
                  >
                    {f.type === "application/pdf" ? (
                      <PictureAsPdfOutlined sx={{ color: "#dc2626" }} />
...
function ReportKpi({ color, label, value, delta, deltaColor }: { color: string; label: string; value: string; delta: string; deltaColor: string }) {
  return (
          ...
          <Paper sx={{ p: 2.5 }}>
            ...
            <Stack spacing={1}>
              {[["Cola Principal", "11", true], ["Asignados a mi", "4", false], ["Recientes", "", false]].map(([l, c, a]) => (
                <Box key={l as string} sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 1.2, borderRadius: 1.5, bgcolor: a ? "rgba(10,61,98,0.08)" : "transparent", border: a ? "1px solid rgba(10,61,98,0.3)" : "1px solid transparent", cursor: "pointer" }}>
                  <Typography sx={{ fontSize: 13, fontWeight: a ? 700 : 500 }}>{l as string}</Typography>
                  {c ? <Chip size="small" label={c as string} sx={{ bgcolor: "primary.main", color: "#fff", fontWeight: 700, height: 22 }} /> : null}
                </Box>
              ))}
            </Stack>
            <Divider sx={{ my: 2 }} />
